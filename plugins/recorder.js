...
const RTSPRecorder = class {
  constructor(config = {}) {
    ...
    this.fileNameFormat = config.fileNameFormat || 'YYYY_M_D_hh_mm_ss'
    this.fileName = null
  ...
  getTodayPath() {
    ...
    return path.join(this.getDirectoryPath(), '')
  }
  ...
  recordStream() {
    ...
    fh.createDirIfNotExists(folderPath)
    const fileName = this.getFilename(folderPath)
    this.fileName = moment().format(this.fileNameFormat) + this.getExtenstion()
    this.writeStream = this.getChildProcess(fileName)
